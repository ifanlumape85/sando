...
<script>
    $(function() {
        ...
        $('select[name=id_propinsi]').val(id_propinsi).change();
        var id_kabupaten = '{{ $item->kelurahan->kecamatan->kabupaten->id ?? null }}';
        var id_kecamatan = '{{ $item->kelurahan->kecamatan->id ?? null }}';
        var id_kelurahan = '{{ $item->kelurahan->id ?? null }}';
            ...
            <div class="col-md-12">
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title">Change TPS</h3>
                    </div>
                    <!-- /.card-header -->
                    <!-- form start -->
                    <form method="post" action="{{ route('tps.update', $item->id) }}">
                        @method('PUT')
                        @csrf
                        @include('pages.tps.partials.form-control', ['submit' => 'Update'])
                    </form>
                </div>
            </div>
        </div>
